...
class AddMeeting {
    constructor() {
        this.title = `Lên lịch họp trực tuyến`;
    }
    ...
    getModalBodyContent() {
                ...
                <div class="input-box">
                    <form id="add-metting-form">
                        <div class="form-row">
                            ...
                            <input id="mt-name" type="text" placeholder="Tên cuộc họp" required>
                        </div>
                        <div class="form-row">
                            <label for="mt-descriptions">Mô tả</label>
                            <textarea name="" id="mt-descriptions" placeholder="Mô tả"></textarea>
                        </div>
                        <div class="form-row">
                            <label for="mt-start-time">Dự kiến bắt đầu vào</label>
                            <input type="datetime-local" name="" id="mt-start-time" required>
                        </div>
                        <div class="form-row">
                            <label for="mt-end-time">Dự kiến kết thúc vào</label>
                            <input type="datetime-local" name="" id="mt-end-time" required>
                        </div>
                        <div class="form-row">
                            <input type="reset" value="Tạo lại">
                            <a id="add-meeting-submit-btn" href="javascript:void(0)" class="btn">Thêm</a>
                        </div>
                        <input type="submit" style="display:none;">
                    </form>
                </div>                            
        ...
        addMeetingForm.addEventListener('submit', event => {
            ...
            RequestHandler.sendRequest('ajax/meeting', {
                classId: getClassId(), name, description, startTime, endTime
            }).then(({ e, m, d }) => {
                if(e) {
                    alert(e); return;
                }
                alert(m);
                document.body.classList.remove('open-modal');
                const meetingContainer = document.querySelector('.meetings');
                if(meetingContainer.querySelector('h3')) meetingContainer.innerHTML = '';
                meetingContainer.innerHTML += d;
            }).catch(error => console.log(error));
        });
        console.log(ctn1);
